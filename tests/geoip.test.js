...
test('normalizes ipwho.is metadata', () => {
  const result = normalizeGeoIp({
    ...
    region: 'Hesse',
    city: 'Frankfurt am Main',
    connection: { asn: 64500, org: 'Example Network' },
...
test('consensus uses the one successful provider when others fail', async () => {
  const fetchImpl = async (url) => {
    if (url.includes('ipwho.example')) {
      return {
        ok: true,
        json: async () => ({
          success: true,
...
test('consensus reports complete agreement', async () => {
  const fetchImpl = async (url) => ({
    ok: true,
...
test('consensus keeps country agreement but reports city disagreement', async () => {
  const fetchImpl = async (url) => ({
    ok: true,
    json: async () => url.includes('ipapi.example')
...
test('consensus reports country disagreement without affecting availability', async () => {
  const fetchImpl = async (url) => ({
    ok: true,
